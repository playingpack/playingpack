...
export function RequestItem({ session, selected, onClick }: RequestItemProps) {
  ...
  const getBorderClass = () => {
    if (session.state === 'pending') return 'border-l-orange-500';
    if (session.state === 'reviewing') return 'border-l-amber-500';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-sm text-gray-200 truncate">{session.request.model || 'unknown'}</span>
      </div>
